...
namespace Maketting.Model
{
    class customerinput_ctrl
    {
        ...
        public void importCustomersoldtolist(object obj)
        {
            ...
            dc.ExecuteCommand("DELETE FROM tbl_MKT_Soldtocode where [Soldtype] = '1' ");
            //    dc.tblFBL5Nnewthisperiods.DeleteAllOnSubmit(rsthisperiod);
            dc.CommandTimeout = 0;
            dc.SubmitChanges();
        ...
        public void customerinputcustomerlist()
        {
        ...
        public void customerinputshiptocode()
        {
            ...
            if (theDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                string filename = theDialog.FileName.ToString();
                Thread t1 = new Thread(importShiptolist);
                t1.IsBackground = true;
                t1.Start(new datainportF() { filename = filename });
