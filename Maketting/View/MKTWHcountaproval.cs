...
{
    public partial class MKTWHcountaproval : Form
    {
        ...
        public int Countingtimes { get; set; }
        //   public int idsub { get; set; }
        LinqtoSQLDataContext datacontext { get; set; }
        IQueryable rs { get; set; }
        ...
        private void btAproval_Click(object sender, EventArgs e)
        {
            ...
            #region change endstock


            var rs3 = from pp in dc.tbl_MKT_Stockends
                     where  pp.Store_code == this.storelocation
                     select pp;
            if (rs3.Count() > 0)
            {
                foreach (var item in rs3)
                {
                 ...
                 var   rs4 = from pp in dc.tbl_MKT_Stockcounts
                          where pp.Store_code == this.storelocation
                          && pp.ITEM_Code == item.ITEM_Code
                          select pp;
                    ...
                    if (rs4.Count() > 0)
                    {
                        ...
                        foreach (var item2 in rs4)
                        {
                            ...
                            item.END_STOCK = item2.CountQuantity + item2.ActualEndatAproval - item2.END_STOCK;
                            dc.SubmitChanges();
                    ...
                    //item.AprovedDate = DateTime.Today;
                    //item.Status = "Aproved";
                    //item.ActualEndatAproval = (from pp in dc.tbl_MKT_Stockcounts
                    //                           where pp.ITEM_Code == item.ITEM_Code
                    //                           && pp.Store_code == this.storelocation
                    //                           select pp.END_STOCK).FirstOrDefault();
                  //  dc.SubmitChanges();
